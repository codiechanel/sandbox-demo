  ...
  render() {
    ...
    if (this.state.items.length > 0 ) {
      panel = this.state.items.map((x, i) => {
        return <div key={i} className={style.masonryLayout__panel}>
            <div className={style.masonryLayout__panelContent}>
            <img className={style.theImage} src={x} alt="Smiley face"  />
            </div>
          </div>
    ...
    return (
       <Paper className="container demo2">
